...
export function PluginPanel({ open, onClose }: { open: boolean; onClose(): void }) {
  ...
  return <div className="absolute bottom-4 right-4 top-16 z-20 w-[520px] overflow-hidden rounded-2xl border bg-card shadow-2xl shadow-black/40">
    ...
    <div className="space-y-3 overflow-auto p-4">
      ...
      <button type="button" className="w-full rounded-xl border border-dashed p-4 text-sm text-muted-foreground hover:border-primary hover:text-foreground"><Download className="mr-2 inline h-4 w-4" />Install plugin from folder / registry coming next</button>
    </div>
  </div>
}
