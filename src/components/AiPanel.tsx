...
export function AiPanel({ open, onClose }: { open: boolean; onClose(): void }) {
  ...
  return <div className="absolute right-4 top-16 z-20 w-[420px] overflow-hidden rounded-2xl border bg-card shadow-2xl shadow-black/40">
    <div className="flex items-center justify-between border-b p-4"><div><div className="flex items-center gap-2 font-semibold"><Bot className="h-4 w-4 text-accent" /> LightPaper AI Lab</div><p className="text-xs text-muted-foreground">Offline stub now, provider plugins later.</p></div><button onClick={onClose}><X className="h-4 w-4" /></button></div>
    <div className="space-y-4 p-4">
      <div className="grid grid-cols-3 gap-2">{presets.map((p) => <button key={p.id} className="rounded-xl border bg-background p-3 text-left text-xs hover:border-primary" onClick={() => run(p.id)}><p.icon className="mb-2 h-4 w-4 text-primary" />{p.label}</button>)}</div>
      ...
      <div className="flex gap-2"><button className="rounded-xl bg-primary px-3 py-2 text-sm text-primary-foreground" onClick={() => run()}><Sparkles className="mr-1.5 inline h-4 w-4" />Run custom prompt</button><button className="rounded-xl border px-3 py-2 text-sm hover:bg-muted" onClick={() => result && setContent(result)}>Replace document</button></div>
