  ...
  return <header className="titlebar-drag flex h-14 shrink-0 items-center justify-between border-b bg-background/72 px-4 backdrop-blur-xl">
    ...
    <div className="titlebar-no-drag flex items-center gap-2">
      <div className="flex rounded-xl border bg-card p-1">
        <button type="button" className={`rounded-lg px-3 py-1.5 text-xs ${settings?.editorMode === 'write' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted'}`} onClick={() => setMode('write')}><Type className="mr-1 inline h-3.5 w-3.5" />Write</button>
        <button type="button" className={`rounded-lg px-3 py-1.5 text-xs ${settings?.editorMode === 'preview' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted'}`} onClick={() => setMode('preview')}><Eye className="mr-1 inline h-3.5 w-3.5" />Preview</button>
        <button type="button" className={`rounded-lg px-3 py-1.5 text-xs ${settings?.editorMode === 'split' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted'}`} onClick={() => setMode('split')}><SplitSquareHorizontal className="mr-1 inline h-3.5 w-3.5" />Split</button>
      </div>
      ...
      <button type="button" title="Settings" className="rounded-xl border bg-card p-2 hover:bg-muted" onClick={onSettings}><Settings className="h-4 w-4 text-muted-foreground" /></button>
    </div>
  </header>
}
